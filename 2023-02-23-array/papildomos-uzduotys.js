...
function mapTask44(arr) {
  ...
  arr.map(item => typeof item === 'number' && console.log(item + 55));
}

// mapTask44(data);

// 4.5. Tik skaičius (number tipo duomenis) ir juos padalinti iš 2.
function forTask45(arr) {
  for (let i = 0; i < arr.length; i++) {
    let item = arr[i];
...
//   "Index: 2, Number: 5"
//   Ir t.t.
function forTask47(arr) {
  for (let i = 0; i < arr.length; i++) {
    if (!isNaN(arr[i])) {
      // let text = 'Index: ' + i + ', Number: ' + arr[i];
      let text = `Index: ${i}, Number: ${arr[i]}`;
      console.log(text);
    }
  }
...
function mapTask47(arr) {
  arr.map((item, index) => {
    if (!isNaN(item)) {
      let text = `Index: ${index}, Number: ${item}`;
      console.log(text);
    }
  });
}
...
function forTask48(arr) {
  for (let i = 0; i < arr.length; i++) {
    if (!isNaN(arr[i])) {
      let answer = i * arr[i];
      let text = `${i} * ${arr[i]} = ${answer}`;
      console.log(text);
    }
  }
}
...
function forTask49(arr) {
  for (let i = 0; i < arr.length; i++) {
    ...
    } else if (typeof currentItem === 'number' && typeof previousItem === 'number') {
      let answer = currentItem * previousItem;
      let text = `${currentItem} * ${previousItem} = ${answer}`;
      console.log(text);
    }
...
function mapTask410(arr) {
  ...
  arr.map(item => !isNaN(item) && item * 5 > 350 && console.log(item));
}

// mapTask410(data);

// 4.11. Tik tekstą (string tipo duomenis) ir prie kiekvieno teksto parašyti kiek simbolių jis turi, pvz. "Start has 5 symbols".
function forTask411(arr) {
  for (let i = 0; i < arr.length; i++) {
    let item = arr[i];

    if (isNaN(item)) {
      let strLength = item.length;
      let firstLetterUpperCaseItem = item[0].toUpperCase() + item.slice(1);
      let text = `${firstLetterUpperCaseItem} has ${strLength} symbols`;
      console.log(text);
    }
  }
}
...
function forTask412(arr) {
  for (let i = 0; i < arr.length; i++) {
    let item = arr[i];

    if (typeof item === 'string') {
      // let answer = item.replaceAll('', '-').slice(1, -1).toUpperCase();
      let answer = item.toUpperCase().split('').join('-');
      console.log(answer);
    }
  }
}
...
function mapTask412(arr) {
  ...
  arr.map(item => typeof item === 'string' && console.log(item.toUpperCase().split('').join('-')));
}
...
function forTask413(arr) {
  for (let i = 0; i < arr.length; i++) {
    let item = arr[i];

    if (typeof item === 'string') {
      let wordLetters = item.split('');
      if (wordLetters.length > 2) {
        // wordLetters[0] = '_';
        // wordLetters[2] = '_';
        wordLetters.splice(0, 1, '_');
        wordLetters.splice(2, 1, '_');
      } else if (wordLetters.length > 0) {
        // wordLetters[0] = '_';
        wordLetters.splice(0, 1, '_');
      }

      let word = wordLetters.join('');
...
function mapTask414(arr) {
  ...
  arr.map((item) => typeof item === 'string' && console.log(item.split('').reverse().join('')));
}
...
function forTask415(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (typeof item === 'string') {
      ...
      if (i === 0) {
        text = `Word ${item} is the first in the array and next item in the array is ${nextItem}.`;
      } else if (i + 1 === arr.length) {
        text = `Word ${item} is the last in the array and previous item in the array is ${previousItem}.`;
      } else { 
        text = `Word ${item} is between ${previousItem} and ${nextItem} in the array`;
      }
