...
function task721(data) {
  ...
  data.forEach(item => {
    let text = `Title: "${item.title}". Content: "${item.body}"`
    let liElement = `<li>${text}</li>`;
    ulElement.innerHTML += liElement;  
  });
}
...
function task722(data) {
  ...
  // let filteredData = data.filter(item => item.title.startsWith('s'));
  
  let filteredData = data.filter(item => item.title.toLowerCase().startsWith('s')); // filtruojamos ir didžiosios ir mažosios raidės
...
function task723(data) {
  ...
  let filteredData = data.filter(item => item.title.toLowerCase().endsWith('t') || item.title.toLowerCase().endsWith('m')); // filtruojamos ir didžiosios ir mažosios raidės
...
function task724(data) {
  ...
  let filteredData = data.filter(item => item.title.length > 15); 
...
function task725(data) {
  ...
  let filteredData = data.filter(item => item.title.length < 20 && item.body.length > 50);
...
function task726(data) {
  ...
  let filteredData = data.filter(item => item.title.length > 20 && item.title.length < 30 && item.body.length > 70 && item.body.length < 100); 
...
function task727(data) {
  ...
  let filteredData = data.filter(item => item.title.includes('it') && item.body.includes('quo')); 
...
function task728(data) {
  ...
  let filteredData = data.filter(item => {
    ...
    let foundWord = words.find(word => word === 'sit');
...
function task729(data) {
  ...
  let filteredData = data.filter(item => {
    let titleSplits = item.title.split('a');
    let bodySplits = item.body.split('o');

    return titleSplits.length > 3 && bodySplits.length > 5;
  });
  
  filteredData.forEach(item => {
    let text = `Title: "${item.title}". Content: "${item.body}"`
    let liElement = `<li>${count}. ${text}</li>`;
    ulElement.innerHTML += liElement;
    count++;  
  });
}
